...
- 📄 **A weekly digest, delivered.** Every Friday at 5pm, the last 7 days' score≥50 winners are written to a one-page PDF (and optionally pushed to a Google Drive folder) — read the week's keepers without opening a single link.
...
automatically every morning (and get the weekly Friday-5pm PDF), see
...
| `poetry run python main.py` | Run the full pipeline: fetch → dedup → triage → deep-eval. Safe to re-run; dedup prevents repeats. No PDF. |
| `poetry run python main.py --digest` | Build + upload the weekly PDF digest (last 7 days of score≥min_score winners). |
...
The pipeline runs **daily** (keeps the backlog fed, spreads the Groq per-day
token budgets across the week) and the PDF digest runs **weekly on Friday at
5pm**. Two separate scheduled tasks point at the Poetry virtualenv's
interpreter (more reliable than `poetry run` from the scheduler). First find
its path:
...
Then in **Task Scheduler → Create Basic Task**, create **two** tasks:

**1. Daily pipeline** — `main.py`
- **Trigger:** Daily, 07:00.
...
**2. Weekly digest** — `main.py --digest`
- **Trigger:** Weekly, Fridays, 17:00.
- **Action:** same program/script, arguments `main.py --digest`,
  same start-in folder.

In each task's properties, enable **"Run task as soon as possible after a
scheduled start is missed"** so it catches up if the PC was asleep.

(macOS/Linux: two `cron` entries —
`0 7 * * * cd /path/ai-radar && poetry run python main.py` and
`0 17 * * 5 cd /path/ai-radar && poetry run python main.py --digest`.)

## 📄 Weekly PDF digest

Every **Friday at 5pm** the score≥min_score winners from the **last 7 days** are
written to a one-page PDF — title, summary, the reason it matched you, tags,
read-time estimate, and the link — so you can read the week's keepers without
opening a single browser tab. It's built by `reporter.py` (using `fpdf2`, no
system deps) via `python main.py --digest` and saved under
`reports/ai-radar-weekly-YYYY-MM-DD.pdf`.

All items **fully evaluated in the trailing 7-day window**
(`evaluated_at` ∈ [now − `window_days`, now)) make it in, so one digest covers
the whole Mon–Fri week instead of a single run's snapshot. The daily `main.py`
run does **not** build a PDF — it only feeds the backlog. The digest step is
best-effort: a PDF or upload failure logs a warning and the job is still
considered successful — reporting must never break the weekly job.
...
authorize once in a browser, and every weekly digest after that uploads (and
refreshes the token) silently — the scheduled job never blocks on a prompt.
Uploaded files are owned by *you* and count against your normal Drive storage.
One-time setup:
...
After that, `poetry run python main.py --digest` uploads the weekly digest
automatically every Friday. (Prefer a service account on Google Workspace? Set
`method: service_account` and supply `GOOGLE_SERVICE_ACCOUNT_FILE` — see
`.env.example`. This path needs a Shared Drive; on a personal Gmail it fails
...
| `report.window_days` | Trailing days the weekly digest covers (default 7). |
